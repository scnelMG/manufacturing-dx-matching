...
- 팀장으로서 데이터 정제, 크롤링·설명 보강, 매칭 결과 검토 흐름을 조율했습니다.
...
> HTML은 GitHub 파일 미리보기보다 브라우저에서 열었을 때 읽기 좋습니다. 실제 결과에 포함된 기업명·공개 기술 설명은 결과 해석에 필요한 범위로만 보존했으며, 원본 학습·수집 데이터는 포함하지 않았습니다.
...
| 개인정보·직접 연락처·주소가 없는 실제 결과 HTML | 원본 기업·특허·크롤링 데이터 |
...
> README의 대표 이미지는 `results/illunex_matching_result.html`을 실제로 렌더링해 캡처한 화면입니다. 임의로 생성한 이미지나 가공된 결과 화면은 사용하지 않았습니다.
